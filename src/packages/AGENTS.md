...
Every classifier now has an on-disk instance, so read one rather than only the record: 45 `.pure.ts`
files and 9 `.impure.ts` files sit under `src/packages/`, measured 2026-09-21. `config-contract/` is
