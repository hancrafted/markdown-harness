...
function assessRun({ path, config, now, root }: Invocation): Termination {
  ...
  const result = assessPath({ root, path }, load.config.sectionFor(frontmatterModule), instant);
...
function checkRun({ root, config }: Invocation): Termination {
  ...
  const checked = checkCorpus(root, files, load.config.sectionFor(frontmatterModule));
