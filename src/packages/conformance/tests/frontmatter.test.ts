...
import type { FrontmatterConfig } from '../../frontmatter-harness/index.ts';
import { queryPath } from '../../frontmatter-harness/query.ts';

const CONFIG_URL = new URL(
  '../../../../fixtures/conformance/modules/frontmatter/valid-test-config.yaml',
  import.meta.url,
);
const parsed = parse(readFileSync(CONFIG_URL, 'utf8')) as { frontmatter: FrontmatterConfig };
 ...
 */
const NON_PAYLOAD_KEYS: readonly string[] = [
  'ruleId',
  'folders',
  'folderTrees',
  'fileNames',
...
describe('valid-test-config.yaml obeys the config-validity rules', () => {
  describe('success cases', () => {
    it('gives every rule at least one selector', () => {
      // ARRANGE
      const selectorKeys = ['folders', 'folderTrees', 'fileNames'];
      // ACT
...
function verdictOf(path: string): string {
  const body = readFileSync(
    new URL(`../../../../fixtures/conformance/modules/frontmatter/${path}`, import.meta.url),
    'utf8',
  );
