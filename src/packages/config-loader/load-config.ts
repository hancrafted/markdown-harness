...
//
// The read itself is not here and is not this Package's any more: bytes arrive
// from `foundation`, the one Package that touches the filesystem, so the config
// and the corpus are read by the same code and cannot disagree about whether a
// path is readable. What is left here is the part that is about a CONFIG —
// which catalog entry each answer earns, and which stage may run next.
//
...
import { readTextAt } from '../foundation/read-text.ts';
...
import { faultForUnread } from './lib/read-fault.pure.ts';
