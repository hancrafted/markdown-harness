...
const CONFIG: FrontmatterConfig = {
  rules: [
    ...
    { ruleId: 'everything', intent: 'The catch-all, written last', folderTrees: ['docs/'] },
    { ruleId: 'inert', intent: 'Reaches nothing in this corpus', folderTrees: ['docs/nothing/'] },
  ],
};
...
describe('auditRules', () => {
  describe('success cases', () => {
    it('tallies the corpus against rules, one row per rule in config order', () => {
    ...
    it('reports a fileNames selector and a folderTrees selector as written', () => {
      // ARRANGE
      const sugar = {
        ruleId: 'index-files',
      ...
      const written = {
        ruleId: 'inert',
        selector: { folderTrees: ['docs/nothing/'] },
  ...
  describe('edge cases', () => {
    it('keeps every row for an empty corpus, so the rules stay visible', () => {
      ...
      // ASSERT
      expect(actual.rules).toHaveLength(ruleCount);
