...
//
// `research` names three folder tokens where it once named one glob, and that
// enumeration IS the grammar: a folder token selects that folder alone, so a
// rule spanning a subtree lists every folder in it. What is under test is the
// bookkeeping across rules, and the list is written out so the counts below can
...
const research: FrontmatterRule = {
  ...
  folders: ['docs/research/', 'docs/research/nested/', 'docs/research/vendor/'],
  excludeFiles: [{ folders: ['docs/research/vendor/'] }],
};

const inert: FrontmatterRule = {
  ruleId: 'inert',
  intent: 'A rule whose folder reaches nothing in this corpus',
  folders: ['docs/nothing/'],
};
 ...
 *
 * `docs/research/vendor/upstream.md` is reached by `research`'s folder list and
 * taken back by its own `excludeFiles`, so it is governed by nothing at all.
...
describe('tallyRules', () => {
  ...
  describe('failure cases', () => {
    it('still reports a rule that governed nothing, which is the point', () => {
      // A rule that wins no file reports nothing anywhere else, so a mistyped
      // folder token is invisible in exactly the direction a trust tool cannot
      // afford.
      // ARRANGE
    ...
    it('shows an ordering mistake as a rule that won nothing it reached', () => {
      ...
      // ARRANGE
      const broad: FrontmatterRule = { ruleId: 'broad', intent: 'Everything', folders: ['docs/'] };
