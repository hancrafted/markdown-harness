...
`npm run verify` is the gate. Six traps inside it.
...
**6. `archgate check` warns on every run, and the warnings are already known.** Its `briefingWarnings` array is non-empty on every invocation, green or red, because four records carry a `Decision` section over the 2,000-character briefing cap — `GEN-001` at 5,232, `ARCH-001` at 4,784, `ARCH-004` at 3,732, `ARCH-003` at 2,199. They are tracked in [#28](https://github.com/hancrafted/markdown-harness/issues/28), and none of them is caused by your change. Do not chase them, and do not report them as a finding. The trap is that they arrive in the same object as real results, so a green run still reads as though something is wrong with it: take `pass`, `failed` and `results` as the verdict and treat `briefingWarnings` as unchanged background until #28 closes. A warning naming a record outside those four, or a section other than `Decision`, is new and does deserve attention.
