...
> **Superseded 2026-09-21 by [`0007-selector-is-two-literal-axes.md`](./0007-selector-is-two-literal-axes.md).**
...
> to refuse. Since #159 the builtin is not called at all: `glob-match.impure.ts` and the
> `GlobMatcher` seam are deleted, and selection compares literal tokens as strings. Kept for the
> measurement, which is still the reason the selector is literal.
